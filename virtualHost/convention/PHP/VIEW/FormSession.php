...
if ($mode === "ajout") {
    ...
    if ($mode == "delete" || $mode == "detail") {
        ...
        for($i = 0;$i<count($listePae);$i++)
        {
        echo '<div class="colonne info titreColonne" >
        <div>
        <div class="colonne" >
        <label for="dateDebutPAE">Date de début de stage: </label>
        <input type="date" name="dateDebutPAE" value="'.$listePae[$i]->getDateDebutPAE().'" disabled/>
        </div>
        <div class="colonne" >
        <label for="dateFinPAE">Date de fin de stage: </label>
        <input type="date" name="dateFinPAE" value="'.$listePae[$i]->getDateFinPAE().'" disabled/>
        </div>
        </div>
        <div class="colonne" >
        <label for="dateRapportSuivi">Date Rapport de suivi: </label>
        <input type="date" name="dateRapportSuivi" value="'.$listePae[$i]->getDateRapportSuivi().'" disabled/>
        </div>
        <div class="colonne" >
        <label for="objectifPAE">Objectif de stage: </label>
        <input type="textarea" name="objectifPAE" value="'.$listePae[$i]->getObjectifPAE().'" disabled/>
        </div>
        </div>';
    }
    } else {
        echo '<select name="idFormation">
                ...
                <div class="colonne" >
                <label for="dateDebutPAE">Date de début de stage: </label>
                <input type="date" name="dateDebutPAE" value="'.$listePae[$i]->getDateDebutPAE().'"/>
                </div>
                <div class="colonne" >
                <label for="dateFinPAE">Date de fin de stage: </label>
                <input type="date" name="dateFinPAE" value="'.$listePae[$i]->getDateFinPAE().'"/>
                </div>
                </div>
                <div class="colonne" >
                <label for="dateRapportSuivi">Date Rapport de suivi: </label>
                <input type="date" name="dateRapportSuivi" value="'.$listePae[$i]->getDateRapportSuivi().'"/>
                </div>
                <div class="colonne" >
                <label for="objectifPAE">Objectif de stage: </label>
                <input type="textarea" name="objectifPAE" value="'.$listePae[$i]->getObjectifPAE().'"/>
                </div>
                </div>';
            }
        }
